# Modified from SQLova to add column sampling
...
def is_picklist(samples, header):
    vals = samples[header]
    return len(set(vals))/len(vals) < 1.1
...
def get_best_samples(query, samples, table_id):
    ...
    for keyword in keywords:
        for header in col_sets:
            is_pick = is_picklist(samples, header)
            if keyword in col_sets[header] and is_pick:
...
def get_fields_1(t1, tables, no_hs_t=False, no_sql_t=False, column_samples = None):
    ...
    if config["use_search_concat"]:
        ...
        hs1 = [x + " ‖ " + " | ".join([y for y in best_samples[x]][:1]) for x in tb1["header"]]
        #print(hs1)
...
def get_bert_output(model_bert, tokenizer, nlu_t, hds, max_seq_length):
    ...
    if len(bert_output) == 3:
        _, pooled_output, all_encoder_layer = bert_output
    elif len(bert_output) == 2:
        # older version
        all_encoder_layer, pooled_output = bert_output
    else:
        # TODO: THROW exception
        all_encoder_layer, pooled_output = None, None
    # 5. generate l_hpu from i_hds
    l_hpu = gen_l_hpu(i_hds)
